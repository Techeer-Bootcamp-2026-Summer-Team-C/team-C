...
Latest 7 days 범위에서 Event를 유형별로 검색하고, 원본 필드와 Process Tree, Raw Payload를 확인합니다.
...
연결된 Alert와 Event를 Investigation Graph로 확장하고, Attack Timeline에서 조사할 근거를 선택합니다.
...
이 과정에서 위험도를 계산하며, MITRE ATT&CK 전술/기술을 연관지어 탐지 결과를 발행하고 이를 알럿과 인시던트로 생성합니다.
...
Grafana Alloy가 호스트, Docker, Kafka, Backend의 메트릭과 Docker 컨테이너 로그를 수집합니다. 메트릭은 Grafana Cloud Metrics로, 로그는 Grafana Cloud Logs로 전송해 운영 상태를 한곳에서 확인합니다.

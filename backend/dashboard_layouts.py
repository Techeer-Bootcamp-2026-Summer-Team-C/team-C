...
OVERVIEW_WIDGET_SPECS: tuple[DashboardWidgetSpec, ...] = (
...
_SPEC_BY_ID = {spec.id: spec for spec in OVERVIEW_WIDGET_SPECS}
...
def default_overview_widgets() -> list[DashboardWidgetLayoutDto]:
    return [
        DashboardWidgetLayoutDto(id=spec.id, x=spec.x, y=spec.y, w=spec.w, h=spec.h, hidden=False)
        for spec in OVERVIEW_WIDGET_SPECS
    ]


def merge_stored_overview_layout(value: object) -> tuple[list[DashboardWidgetLayoutDto], bool]:
    if not isinstance(value, list):
        return default_overview_widgets(), False

    seen: set[str] = set()
    merged: list[DashboardWidgetLayoutDto] = []
    registry_order = {spec.id: index for index, spec in enumerate(OVERVIEW_WIDGET_SPECS)}
    for raw in value:
        if not isinstance(raw, dict):
            continue
        widget_id = raw.get("id")
        if not isinstance(widget_id, str) or widget_id in seen or widget_id not in _SPEC_BY_ID:
            continue
        seen.add(widget_id)
        spec = _SPEC_BY_ID[widget_id]
...
def validate_overview_layout(body: DashboardLayoutPutRequest) -> list[DashboardWidgetLayoutDto]:
    if body.layout_version != DASHBOARD_LAYOUT_VERSION:
    ...
    for widget in body.widgets:
        ...
        seen.add(widget.id)
        spec = _SPEC_BY_ID.get(widget.id)
        if spec is None:
    ...
    return merge_stored_overview_layout([widget.model_dump() for widget in validated])[0]
...
class DashboardLayoutService:
    ...
    def get(self, *, user_id: int, dashboard_key: str) -> DashboardLayoutDto:
        ...
        if stored is None:
            return _response(revision=0, is_default=True, widgets=default_overview_widgets())
        if int(stored.get("layout_version", 0)) > DASHBOARD_LAYOUT_VERSION:
            widgets, usable = default_overview_widgets(), False
        else:
            widgets, usable = merge_stored_overview_layout(stored.get("layout_json"))
        return _response(revision=int(stored["revision"]), is_default=not usable, widgets=widgets)
    ...
    def put(
        ...
        *,
        ...
        return _response(revision=revision, is_default=False, widgets=widgets)
...
def _response(
    *, revision: int, is_default: bool, widgets: list[DashboardWidgetLayoutDto]
) -> DashboardLayoutDto:

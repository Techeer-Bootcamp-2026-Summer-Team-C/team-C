...
function OverviewContent({ data, layoutResponse, layoutLoadError, layoutLoadedAt, layoutLoading, onReloadLayout }: {
  ...
  const editor = useDashboardLayoutEditor(layoutResponse, layoutLoadedAt);
  ...
  const hiddenTrayHint = breakpoint === "md"
    ? (hiddenWidgets.length ? "Select a hidden widget to restore it. Arrange its position at desktop width." : "Widget arrangement is available at desktop width.")
    : (draggingWidgetId ? "Drop the card here to hide it." : hiddenWidgets.length ? `${hiddenWidgets.length} hidden - drag a widget back onto the grid` : "Drag a card here to hide it.");
  const canStartEditing = breakpoint === "lg" && !layoutLoading && !layoutLoadError;
  const editorHint = editor.isEditing
    ? (breakpoint === "lg" ? "Drag anywhere on a card, resize from its corner, or use the keyboard." : "The tablet layout is generated from the saved desktop order.")
    : layoutLoading
      ? "Loading your saved dashboard layout."
      : breakpoint === "md"
        ? "The tablet layout follows your saved desktop order."
        : "Drag, resize, hide, and restore content widgets.";
  ...
  return (
    <>
      <section className={`dashboard-editor-bar ${editor.isEditing ? "editing" : ""}`} aria-label="Dashboard layout controls">
        <div>
          <strong>{editor.isEditing ? "Dashboard editing" : "Dashboard layout"}</strong>
          <span>{editorHint}</span>
        </div>
        <div className="dashboard-editor-actions">
          {editor.isEditing ? <>
            <button className="button ghost dashboard-editor-only" disabled={editor.saveStatus === "saving"} onClick={editor.cancelEditing} type="button"><X aria-hidden="true" size={15} />Cancel</button>
            <button className="button ghost dashboard-editor-only" disabled={editor.saveStatus === "saving"} onClick={() => void editor.resetLayout()} type="button"><RotateCcw aria-hidden="true" size={15} />Reset default</button>
            <button className="button dashboard-editor-only" disabled={editor.saveStatus === "saving"} onClick={() => void editor.saveNow()} type="button"><Save aria-hidden="true" size={15} />Save</button>
            <button className="button primary dashboard-editor-only" onClick={editor.finishEditing} type="button">Done</button>
          </> : <button className="button dashboard-edit-button" disabled={!canStartEditing} onClick={editor.startEditing} type="button"><Settings2 aria-hidden="true" size={15} />Edit dashboard</button>}
        </div>
      </section>
      {layoutLoadError ? <div className="dashboard-layout-message error" role="alert"><span>Saved layout could not be loaded. The default layout is shown.</span><button className="button ghost" onClick={() => void onReloadLayout()} type="button"><RefreshCw aria-hidden="true" size={15} />Reload</button></div> : null}
      {editor.conflict ? <div className="dashboard-layout-message conflict" role="alert"><span>This layout changed in another screen. Reload the latest revision before editing again.</span><button className="button" onClick={() => void editor.reload(onReloadLayout)} type="button"><RefreshCw aria-hidden="true" size={15} />Reload latest</button></div> : null}
      {editor.errorMessage ? <div className="dashboard-layout-message error" role="alert">{editor.errorMessage}</div> : null}
      <div className="dashboard-save-status" aria-live="polite" role="status">{saveStatusText(editor.saveStatus)}</div>
      {editor.isEditing ? <aside
        aria-label="Hidden widget drop zone"
        className={`hidden-widget-tray dashboard-editor-only ${hideDropActive ? "drop-active" : ""}`}
        ref={hideZoneRef}
      >
        <div><strong><EyeOff aria-hidden="true" size={16} />Hidden widgets</strong><span>{hiddenTrayHint}</span></div>
        {hiddenWidgets.length ? <ul>{hiddenWidgets.map((item) => {
          const title = OVERVIEW_WIDGET_BY_ID.get(item.id)?.title ?? item.id;
          return <li key={item.id}><button
            aria-label={desktopEditing ? `Restore ${title}. Drag it onto the dashboard or press Enter.` : `Restore ${title}. Press Enter.`}
            ...
            type="button"
          ><GripVertical aria-hidden="true" size={15} /><span>{title}</span><small>{desktopEditing ? "Drag to restore" : "Select to restore"}</small></button></li>;
        })}</ul> : <span className="hidden-widget-empty">No hidden widgets</span>}
      </aside> : null}
        ...
        >
          {OVERVIEW_WIDGET_REGISTRY.filter((registration) => !editor.draft.find((item) => item.id === registration.id)?.hidden).map((registration) => {
            ...
            return <div className={`dashboard-widget ${mode}`} data-widget-id={registration.id} key={registration.id}>
              ...
              /> : null}
              <div className="dashboard-widget-content">{registration.render(data, mode)}</div>
            </div>;
          })}
...
function WidgetDragSurface({ widgetId, title, hideable, onHide, onMove, onResize }: {
  ...
  return <>
    ...
    <span className="sr-only" id={`${widgetId}-keyboard-help`}>Drag to move. Alt plus arrow moves order. Alt Shift plus arrow changes width or height. Delete hides this widget.</span>
  </>;
}
...
export function useDashboardLayoutEditor(
  ...
  const persistNow = useCallback(async (candidate: DashboardWidgetLayout[]) => {
    ...
    try {
      ...
      if (error instanceof ApiError && error.status === 409) {
        ...
        setErrorMessage(error instanceof Error ? error.message : "The dashboard layout could not be saved.");
        setSaveStatus("error");
      }
    } finally {
  ...
  return {
    ...
    saveNow,
    async resetLayout() {
      if (!window.confirm("Reset this dashboard to the default layout? Your saved layout will be deleted.")) return;
      if (saveTimerRef.current) clearTimeout(saveTimerRef.current);
      saveTimerRef.current = null;
      queuedRef.current = null;
      setSaveStatus("saving");
      try {
        ...
        setDraft(committedRef.current);
        setSaveStatus("error");
        setErrorMessage(error instanceof Error ? error.message : "The default layout could not be restored.");
      }
    },
...
function saveStatusText(status: SaveStatus): string {
  if (status === "unsaved") return "Layout changes waiting to save.";
  if (status === "saving") return "Saving dashboard layout.";
  if (status === "saved") return "Dashboard layout saved.";
  if (status === "error") return "Dashboard layout save failed. The last saved layout was restored.";
  if (status === "conflict") return "Dashboard layout save conflict. Reload the latest revision.";
  return "";
}

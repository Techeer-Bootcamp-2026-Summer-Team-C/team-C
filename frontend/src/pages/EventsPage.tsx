...
export function EventsPage() {
  ...
  return <div className="page-stack">
    ...
    })}</tbody></DataTable><Pagination page={result.data.data} /></> : <EmptyState actions={<><button className="button ghost" disabled={!appliedFilters.length} onClick={() => setParams({})} type="button">{t("filter.clear")}</button><button className="button primary" onClick={() => setParams(updateParams(params, { timePreset: "LATEST_7D", from: null, to: null, page: null }))} type="button">{t("filter.latest7Days")}</button></>} title={t("events.noResults")} message={appliedFilters.length ? t("events.noFilterMatch") : t("events.noneInRange")} />}</Panel> : null}
  </div>;
}

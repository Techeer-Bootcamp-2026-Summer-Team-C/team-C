...
export function AlertDetailPage() {
  ...
  return <div className="page-stack">
    <Link className="back-link" to="/alerts"><ArrowLeft aria-hidden="true" size={15} />Alert queue</Link>
...
function AlertDetail({ alert, canUpdate, mutation }: {
  ...
  return <>
    ...
    <section className="detail-grid">
      ...
      <Panel title="Workflow state" subtitle={canUpdate ? "ADMIN and ANALYST may update Alert status" : "VIEWER access is read-only"}>{canUpdate ? <Field label="Alert status"><select disabled={mutation.isPending} onChange={(event) => mutation.mutate(event.target.value as AlertStatus)} value={alert.status}><option>OPEN</option><option>IN_PROGRESS</option><option>RESOLVED</option></select></Field> : <div className="read-only-note"><StatusPill value={alert.status} /><span>Status controls are hidden for VIEWER.</span></div>}</Panel>

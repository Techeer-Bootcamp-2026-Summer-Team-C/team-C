  ...
  return <section className="triage-workspace">
    ...
    <div className="triage-detail page-stack">
      ...
      <DetailLedger className="alert-evidence-ledger">
        ...
        <DetailLedgerSection title={t("alert.connectedIncidents")} subtitle={t("alert.correlationReferences")}>{alert.incidents.length ? <div className="link-list connected-incident-list">{alert.incidents.map((incident) => <Link key={incident.incidentId} to={`/incidents/${incident.incidentId}`}><span className="connected-incident-copy"><strong>{detectionTitle(t, incident.title)}</strong><small>{formatDateTime(incident.windowStartAt)} – {formatDateTime(incident.windowEndAt)}</small></span><span className="connected-incident-badges"><StatusPill value={incident.severity} /><StatusPill value={incident.status} /></span></Link>)}</div> : <EmptyState title={t("alert.noConnectedIncidents")} message={t("alert.noConnectedDescription")} />}</DetailLedgerSection>
      </DetailLedger>

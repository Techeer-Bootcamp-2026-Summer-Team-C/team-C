...
export function ProcessTree({ nodes }: { nodes: readonly ProcessTreeNodeDto[] }) {
  ...
  if (!rendered.length) {
    return <EmptyState title="No process lineage captured" message="No PROCESS_EXECUTION events were collected in the surrounding 35-minute window." />;
  }
  return <div className="process-tree" role="tree" aria-label="Collected process lineage">
    ...
    >
      ...
      <span className="process-copy">
        ...
        <code>{node.commandLine ?? node.processPath ?? "Command line unavailable"}</code>
        <small>{formatDateTime(node.lastSeenAt)} · PID {node.pid} · PPID {node.ppid ?? "None"} · {node.eventCount} event(s)</small>
      </span>

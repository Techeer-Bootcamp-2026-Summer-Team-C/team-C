...
def test_product_api_manifest_contains_exactly_29_unique_contracts() -> None:
    assert len(PRODUCT_API_CONTRACTS) == 29
    method_paths = {(contract.method, contract.path) for contract in PRODUCT_API_CONTRACTS}
    assert len(method_paths) == 29
    assert sum(contract.path.startswith("/collector/") for contract in PRODUCT_API_CONTRACTS) == 3
    assert sum(not contract.path.startswith("/collector/") for contract in PRODUCT_API_CONTRACTS) == 26

...
2. 시각·레이아웃·상호작용: 이 문서
3. 이번 개편의 범위·순서·진행 상태: `docs/frontend/FRONTEND_UI_REDESIGN_PLAN.md`
4. Route·query·polling·권한 동작: `docs/frontend/FRONTEND_SPEC.md`
5. 현재 구현: `frontend/src/`
6. 아이디어·회의 자료: 리디자인 Workshop 문서
...
| `FRONTEND_UI_REDESIGN_WORKSHOP.md` | 화면 구조 선택지와 미결 질문 참고 |
| `FRONTEND_UI_REDESIGN_DISCUSSION.md` | 기존 링크 호환용 참고 |
| `FRONTEND_UI_REDESIGN_DECISION_QUESTIONS.md` | 기존 링크 호환용 참고 |
...
기존 파일은 이 문서 작성 과정에서 삭제하거나 이름을 변경하지 않는다.
...
이미지 원본은 현재 repo 외부의 팀 작업 자료다. 장기 보존이나 PR 리뷰에 사용할 경우 별도 승인 후 `docs/frontend/assets/references/`로 옮기고 출처와 용도를 함께 기록한다. 토의 메모에 포함된 `Image20260715211828.png`는 이번 확인 자료에 없어 해석하지 않는다.
...
현재 구현의 문제 목록, 우선순위와 처리 상태는 단발성 정보이므로 [Frontend UI 개편 실행 계획](./FRONTEND_UI_REDESIGN_PLAN.md)의 `Baseline 문제 목록`에서 관리한다. 이 문서에는 장기 목표 디자인과 유지할 기반만 남긴다.
...
- cyan은 primary action과 keyboard focus에 제한
- red, amber, green, blue는 의미가 정해진 상태에만 사용
- 목적이 명확한 gradient는 제한적으로 사용하고, 무의미한 glow와 glass effect는 사용하지 않음
- panel 수보다 정보 위계와 whitespace로 구획
- 이번 개편은 dark-only로 배포하되 semantic token은 future light theme를 추가할 수 있게 역할 기반으로 정의

### 6.2 Functional gradient — `확정`

Gradient는 허용한다. 다만 색상 장식 자체가 아니라 정보의 깊이, 선택된 조사 경로 또는 정량 chart의 연속성을 보조해야 한다.

허용:

- chart area에서 series color가 투명 surface로 사라지는 fill
- 선택된 evidence, investigation path 또는 주요 KPI 영역의 낮은 대비 accent surface
- page 또는 panel의 한정된 영역에서 시선의 시작점을 만드는 subtle background

금지:

- 모든 card와 panel에 동일 gradient 반복
- critical, warning, success 상태를 gradient만으로 표현
- text contrast를 낮추는 강한 multi-color 배경
- 빈 공간을 채우기 위한 AI-purple mesh, ambient glow, glassmorphism

한 화면에서 accent gradient family는 하나만 사용한다. semantic status color는 gradient와 분리하며, text가 올라가는 경우 각 viewport에서 WCAG AA contrast를 검증한다.

### 6.3 Color 역할 — `확정`

아래 색상값을 구현 baseline으로 사용한다. 각 component와 viewport에서 contrast를 검증하고 접근성 기준을 만족하지 못하는 값만 의미를 유지한 채 조정한다.

| Semantic token | Prototype baseline | 역할 |
| --- | --- | --- |
| `--surface-canvas` | `#101215` | 전체 배경 |
| `--surface-shell` | `#0d0f12` | Navigation, top bar |
| `--surface-panel` | `#171c22` | 기본 panel |
| `--surface-raised` | `#202733` | popover, selected surface |
| `--surface-inset` | `#11161c` | input, code, nested block |
| `--border-default` | `#323a46` | control과 panel 경계 |
| `--border-subtle` | `#272e38` | 내부 구분선 |
| `--text-primary` | `#eef2f7` | 제목과 핵심 값 |
| `--text-secondary` | `#9aa8ba` | 설명과 보조 정보 |
| `--text-tertiary` | `#748296` | 낮은 우선순위 meta |
| `--accent-primary` | `#56d0e6` | primary action과 focus |
| `--status-critical` | `#f2495c` | Critical, unavailable, destructive error |
| `--status-warning` | `#ffb357` | High, degraded, stale |
| `--status-success` | `#73bf69` | Healthy, completed |
| `--status-info` | `#5794f2` | Medium, selected evidence |
| `--gradient-accent-surface` | cyan/blue에서 transparent로 이어지는 저대비 gradient | 선택 영역과 주요 조사 시작점 |
| `--gradient-chart-fill` | series color에서 transparent로 이어지는 세로 gradient | area chart의 값과 baseline 연결 |
...
UI는 `Inter`, `Segoe UI`, system UI 순서, ID·hash·path·raw payload는 `Cascadia Code`, `Consolas`, monospace 순서를 사용한다.
...
- desktop dashboard는 12-column grid와 12px gutter, tablet·mobile은 8px gutter를 prototype baseline으로 사용한다.
...
```text
[Endpoint scope] [Time range ▾] [Refresh ▾] [Last updated] … [Edit layout]
```

- Time range, refresh interval과 layout option은 항상 펼쳐진 큰 bar가 아니라 button + Popover 또는 Drawer로 제공한다.
...
- `Edit layout`은 명시적인 edit mode로 진입하고 `Unsaved`, `Saving`, `Saved`, `Conflict` 상태를 구분한다.
- `Done`은 pending save가 완료된 뒤 edit mode를 종료한다. 저장 중 refresh·이탈은 완료 대기 또는 사용자 경고로 결정적으로 처리한다.
...
- records는 drag reorder하지 않는다. drag는 사용자가 직접 배치하는 dashboard layout에만 허용한다.
...
- EDR state와 주요 원인
- Alerts, Open Incidents, High-risk Endpoints, Event Failures KPI
- Detection activity
- Alert severity
...
KPI card는 8.6의 anatomy를 따르며 API가 비교값을 제공하지 않으면 previous value와 delta를 만들지 않는다. Time range, Endpoint scope, refresh와 layout edit는 7.3의 compact toolbar에 둔다.
...
기본 Overview는 위 10개 block으로 제한한다. 위 8개 widget은 기본값에서 제거하고 기존 사용자 저장 layout은 새 layout version으로 1회 자동 migration한 뒤 변경 안내를 표시한다. Events·Online Endpoints·Storage·Guidance 요약은 각각 Detection Activity·Collection Health·Archives·Alert Detail로 이동하거나 통합한다. 팀 이미지의 realtime event stream은 Overview 기본 block으로 직접 복제하지 않는다.
...
If the task is part of the current redesign, read docs/frontend/FRONTEND_UI_REDESIGN_PLAN.md and work on exactly one assigned Work Package.
...
5. Do not implement any item marked Blocked in the active redesign plan.

After editing:
Run the section 14.7 pre-flight and report evidence for each applicable item.
Update only the assigned Work Package status and evidence in the active redesign plan.
```
...
이 문서는 장기 디자인 기준만 관리한다. 이번 개편의 작업 순서, 담당, 진행 상태, blocker, 구현 파일과 완료 증거는 [Frontend UI 개편 실행 계획](./FRONTEND_UI_REDESIGN_PLAN.md)에서 관리한다.
...
- 개편이 끝나면 실행 계획은 완료 기록으로 보존하거나 archive하고, 이 문서는 계속 갱신한다.
...
| D-007 | 2026-07-15 | 장기 디자인 기준과 단발성 개편 계획을 분리 | `DESIGN.md`의 지속 가능성을 유지하고 실행 진행률과 blocker의 수명을 분리 | `docs/frontend/DESIGN.md`, `docs/frontend/FRONTEND_UI_REDESIGN_PLAN.md`, `frontend/AGENTS.md` |
...
| D-011 | 2026-07-15 | Workshop의 모든 권장안을 확정하고 Login을 포함한 dark-only compact UI로 개편 | 구현 중 반복 의사결정을 제거하고 화면 간 일관성을 고정 | `docs/frontend/DESIGN.md`, `docs/frontend/FRONTEND_UI_REDESIGN_WORKSHOP.md` |
| D-012 | 2026-07-15 | Overview 10개 block, 8개 widget 제거와 1회 자동 layout migration을 확정 | Overview를 상태·원인·우선 조사 대상에 집중 | `docs/frontend/DESIGN.md`, `docs/frontend/FRONTEND_UI_REDESIGN_PLAN.md` |
...
- [Frontend UI 개편 실행 계획](./FRONTEND_UI_REDESIGN_PLAN.md)
- [Frontend·Backend 통합 작업 지시서](./FRONTEND_BACKEND_UI_REDESIGN_WORK_ORDER.md)
- [Frontend UI 개편 결정 Workshop](./FRONTEND_UI_REDESIGN_WORKSHOP.md)

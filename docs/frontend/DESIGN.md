...
2. 시각·레이아웃·상호작용: 이 문서
3. Route·query·수동 갱신·권한 동작: `docs/frontend/FRONTEND_SPEC.md`
4. 현재 구현: `frontend/src/`
5. 종료된 아이디어·실행 계획·회의 자료: Git history
...
현재 Overview의 장기 품질 기준은 이 문서와 `FRONTEND_SPEC.md`, 실제 상태는 코드와 테스트에서 관리한다. 완료된 구현 계획과 검증 증거는 Git history에서 확인한다.
...
| --- | --- |
| `/` | 전체 상태와 조사 시작점 |
| `/dashboards` | 사용자별 Dashboard layout 생성·편집·삭제 |
| `/alerts` | Alert 분류와 처리 |
...
- `/dashboards`는 Primary navigation에 노출하지 않고 Overview toolbar의 `Dashboard 설정` action으로 진입하는 보조 설정 route다.
...
- 인증된 AppShell에서 dark/light를 전환하고 `edr.theme`에 저장한다. 저장값이 없거나 손상되면 Case 2 dark token을 기본값으로 사용한다.
...
### 6.3 Color 역할 — `값 확정 / Case 2 dark baseline + semantic light`
...
- Default Overview desktop은 목적별 고정 CSS grid와 12px gutter를 사용한다. 사용자별 Custom Dashboard의 12-column 배치 grid는 `/dashboards` Draft 편집기에서만 사용하고 `/`에서는 읽기 전용으로 렌더링한다.
...
[ Detection Activity 2fr ][ Fleet Distribution 1fr ]
...
- `/` Overview는 active Dashboard를 읽기 전용으로 표시한다. Default가 active이면 10-block을 고정 DOM 순서로 전부 표시하고, Custom Dashboard가 active이면 저장된 9개 widget 조합을 표시하며 Default로 되돌리는 action을 제공한다. `/`에서는 어느 layout도 drag, drop, resize, hide 또는 저장할 수 없다. Fleet Distribution은 Default 관제 context이며 `/dashboards` widget catalog에는 추가하지 않는다. `/dashboards` workbench의 명시적 Draft 편집기에서만 기존 9개 widget의 drag/drop, resize와 삭제를 제공하고 Save 시 사용자별 browser-local layout을 한 번 저장한다.
...
- table record와 Default dashboard block은 drag reorder하지 않는다. 사용자가 생성한 custom dashboard widget만 명시적 edit mode와 drag handle에서 reorder한다.
...
- EDR state와 주요 원인
- Total Alerts, Critical Alerts, HIGH-level Endpoints, Open Incidents KPI. `highRiskEndpointCount`는 Critical 합산값이 아닌 정확한 `HIGH` 등급이므로 label에도 등급을 명시한다.
- Detection activity
- Alert severity
- Fleet distribution: Endpoint risk level과 현재 Sensor Health
- Highest-risk Endpoints
- Incident queue

KPI card는 8.6의 anatomy를 따르며 API가 비교값을 제공하지 않으면 previous value와 delta를 만들지 않는다. Fleet distribution은 `EndpointSummaryDto.risk.byLevel`과 `sensorHealth`의 현재 집계만 사용하고 시간 추세처럼 표현하지 않는다. Time range, Endpoint scope와 refresh는 7.3의 compact toolbar에 둔다.

신규 기본 Overview에서 제거할 7개:

1. Endpoint operating systems
2. Top rules
3. MITRE detection distribution
4. Process and network signals
5. File, DNS, and L7 signals
...
기본 Overview는 위 10개 block을 immutable `Default` dashboard로 유지한다. 1440px에서는 Detection Activity 왼쪽과 Alert Severity·Fleet Distribution 오른쪽 stack으로, 1720px 이상에서는 세 분석 panel을 한 행으로 표시한다. 별도 custom dashboard는 기존 9개 block만 widget catalog로 재사용하며 Fleet Distribution은 Default Overview context로 제한한다. 선택한 custom layout은 `/`에서 읽기 전용으로 적용하고 Default 복원 action을 항상 제공한다. 사용자 layout은 browser localStorage에 저장한다. Backend layout API와 저장 데이터는 호환성을 위해 유지하되 Frontend에서 호출하거나 migration하지 않는다. Events·Online Endpoints·Storage·Guidance 요약은 각각 Detection Activity·Collection Health·Archives·Alert Detail로 이동하거나 통합한다. 팀 이미지의 realtime event stream은 Overview 기본 block으로 직접 복제하지 않는다.
...
- 선택 node·edge·row 강조
- 저장, retry, 수동 refresh 상태의 짧은 feedback

### 11.2 금지 — `확정`

- 수동 refresh마다 chart 전체가 다시 등장하는 animation
...
5. For a broad redesign, define a scoped implementation plan before editing.

After editing:
Run the section 14.7 pre-flight and report evidence for each applicable item.
Record durable product decisions in DESIGN.md or FRONTEND_SPEC.md.
```
...
이 문서는 장기 디자인 기준만 관리한다. 일회성 작업 순서와 실행 증거는 scoped plan에서 관리하고, 완료 후 확정된 기준만 이 문서에 이관한다.
...
- 개편이 끝나면 실행 계획은 Git history에 보존하고 working tree에서는 제거하며, 이 문서는 계속 갱신한다.
...
| D-007 | 2026-07-15 | 장기 디자인 기준과 단발성 개편 계획을 분리 | `DESIGN.md`의 지속 가능성을 유지하고 실행 진행률과 blocker의 수명을 분리 | `docs/frontend/DESIGN.md`, Git history의 완료 계획, `frontend/AGENTS.md` |
...
| D-015 | 2026-07-16 | Overview를 승인 시안 기반 고정형 10-block dashboard로 전환하고 Frontend layout 편집·저장·migration을 제거 | 자유 배치보다 상태·원인·우선 조사 대상의 일관된 정보 위계가 중요하며 구현·검증 복잡도를 줄임 | Git history의 완료 계획, `frontend/src/pages/OverviewPage.tsx` |
...
| D-017 | 2026-07-17 | Overview를 EDR command strip, KPI 4개, Detection Activity, Alert Severity donut, 조사 queue 2개의 고정 9-block으로 조정하고 Endpoint Risk panel 제거 | 위험 표현 중복과 상단 dead space를 줄이고 현재 상태와 조사 대상을 빠르게 판독 | Git history의 완료 계획, `frontend/src/features/overview/OverviewDashboard.tsx` |
...
| D-025 | 2026-07-17 | Case 2 dark를 기본값으로 유지하면서 semantic light theme와 사용자별 browser-local custom Overview dashboard를 제공한다. Default 9-block Overview는 immutable로 유지하고 custom dashboard에서만 desktop drag/drop·resize를 허용한다 | 사용자가 Team B reference의 theme와 dashboard builder interaction을 요청했다. 현재 API·DTO·Backend layout 계약을 보존하고 모바일 시각 QA는 제외한다 | Git history의 완료 계획, `frontend/src/theme/`, `frontend/src/features/overviewLayout/` |
| D-026 | 2026-07-18 | Overview는 선택된 Dashboard의 읽기 전용 화면으로 유지하고 생성·이름 변경·Widget 이동/resize·삭제를 `/dashboards` workbench의 명시적 Draft 편집기로 분리한다. 본문의 관리 box 대신 header의 `Dashboard 설정` modal이 layout 선택과 관리 action을 소유한다 | 탐색과 편집 책임을 분리해 reference와 다른 제품 구조를 만들고, 이동 종료마다 즉시 저장되던 오류 가능성을 제거해 Save/Cancel 경계를 명확히 한다 | `frontend/src/App.tsx`, `frontend/src/pages/OverviewPage.tsx`, `frontend/src/features/overviewLayout/OverviewDashboardWorkspace.tsx` |

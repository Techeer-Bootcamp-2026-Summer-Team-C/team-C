...
이 제품은 빠르게 이상 상태를 파악하고 근거 데이터로 이동할 수 있는 조용한 보안 관제 콘솔이다. 시그니처는 244px 기본 Sidebar, Signal → Evidence → Decision 상단 경로, royal-blue active navigation, dark-only near-black surface와 제한된 상태 색상이다. 화면은 마케팅 Dashboard처럼 과장하지 않고, 숫자·상태·최근성·다음 이동 경로를 우선한다.
...
Dark-only semantic token을 제공한다. Theme 전환이나 `edr.theme` 저장은 제공하지 않는다.

...
- `localStorage`에는 마지막 primary route와 navigation compact 상태만 저장하며 별도 locale key를 만들지 않는다. Overview layout은 JWT 사용자별 Backend API와 PostgreSQL `user_dashboard_layouts`에 저장한다.
- Overview grid는 `react-grid-layout@2.2.3`의 drag, resize, collision 재배치, vertical compaction과 bounds 기능을 사용한다.
...
| 409 | resource 상태 오류 표시, `ARCHIVE_NOT_READY`는 Archive route 안내, `DASHBOARD_LAYOUT_REVISION_CONFLICT`는 최신 layout 다시 불러오기 제공 |
...
| 본인 Overview layout 조회·저장·초기화 | 허용 | 허용 | 허용 |
...
- `GET /dashboard/layouts/overview`

표시:

- `EdrStatePill`: status, score, reason codes, calculatedAt
- KPI: Alert, OPEN Incident, High-risk Endpoint, Event failure
- Detection Activity: Backend가 제공한 Event, Alert, Incident time bucket의 통합 표시
- Alert severity donut
- Endpoint Risk distribution과 highest score
- Highest-risk Endpoint와 OPEN Incident queue
- EDR state를 포함한 승인된 10개 block만 Overview registry가 소유한다.
- Endpoint scope 제어는 현재 Summary API 계약이 지원하는 `All endpoints`만 표시하며 client-side 가상 필터를 만들지 않는다.
- `lastRefreshedAt`

KPI와 chart 선택은 관련 목록 route로 이동하며 가능한 경우 URL filter를 설정한다.

Overview의 콘텐츠 카드는 안정 ID registry로 관리한다. version 2 registry는 `edr-state`, `kpi-alerts`, `kpi-open-incidents`, `kpi-high-risk-endpoints`, `kpi-event-failures`, `detection-activity`, `alert-severity`, `endpoint-risk`, `highest-risk-endpoints`, `incident-queue`의 10개다. 헤더, sidebar, 사용자 menu, route와 위젯 내부 제목·버튼·범례는 layout 단위가 아니다. 기본 화면을 먼저 표시하고 저장 layout 응답을 registry와 병합한다.

- 저장된 version 1 layout은 제거 widget을 제외하고 version 2 신규 기본 block을 병합해 한 번 자동 migration한다. migration PUT 성공 후에만 사용자 안내 완료 상태를 현재 탭에 기록한다.
- `409 DASHBOARD_LAYOUT_REVISION_CONFLICT`에서는 최신 layout을 다시 GET하고 같은 migration pure function을 재적용할 수 있어야 한다. Backend는 version을 자동 변경하지 않는다.
- version 2 save와 reload가 성공하기 전에는 version 1 원본을 migration 완료로 간주하지 않는다.

- 데스크톱은 12열이며 편집 mode에서만 카드 전체 표면 drag와 corner resize를 허용한다.
- 기본 layout은 즉시 표시하되 저장 layout 조회가 끝날 때까지 편집 진입을 잠근다. 조회 실패 시 오류와 다시 불러오기 action을 표시하고 편집을 시작하지 않는다.
- 태블릿은 저장된 데스크톱 순서를 6열로 자동 변환한 읽기 전용 layout이며 drag, resize와 편집 진입을 제공하지 않는다.
- 위젯마다 콘텐츠에 맞는 `minW/minH/maxW/maxH`와 hideable 설정을 사용한다. KPI는 1열 compact를 허용하고 time-series는 최소 6열, 일반 panel/list는 3~6열의 실제 최소 폭을 사용한다.
- layout item은 `id`, `x`, `y`, `w`, `h`, `hidden`을 가진다. 중복/삭제 ID 제거, 신규 위젯 기본 위치 추가, bounds와 크기 보정, 겹침 제거와 위쪽 압축은 순수 함수로 처리한다.
- 너비/높이에 따라 `compact`, `standard`, `expanded` mode를 결정한다. compact KPI는 보조 설명을 숨기고, compact chart는 point 밀도를 줄이며 전체 값/table은 details에서 확인한다. expanded chart는 추가 통계를 표시한다.
- compact 분포는 상위 3개, standard 분포는 상위 4개를 우선 표시하고 나머지는 같은 카드의 details에서 확인한다. 긴 hash, hostname과 signal label은 한 줄 말줄임으로 보호하되 accessible text와 tooltip에는 전체 값을 유지한다.
- 시계열 chart는 카드의 실제 남은 높이를 사용해 축소·확대하고, compact EDR은 state와 score를 우선 표시한다. 허용된 최소·최대 크기에서 widget content와 panel body는 가로·세로 영역을 넘지 않아야 한다.
- 편집 mode에서는 카드 전체 drag overlay가 chart, link, table 조작을 차단하고 일반 mode에서는 overlay를 제거해 기존 조작을 유지한다.
- drag surface에는 접근성 이름을 제공하고 Alt+Arrow로 순서, Alt+Shift+Arrow로 width/height 단계를 바꾸며 Delete/Backspace로 숨긴다.
- drop, resize 종료, 숨김, 복원은 650ms debounce 후 `PUT /dashboard/layouts/overview`로 전체 layout을 저장한다. 저장 성공 시 revision을 갱신하고 실패 시 마지막 정상 layout으로 복구한다.
- `revision` 충돌은 다른 화면 변경 message와 최신 layout 다시 불러오기 action을 표시한다. 현재 편집 취소는 편집 시작 layout으로 복귀하고, 기본값 초기화는 확인 후 `DELETE /dashboard/layouts/overview`를 호출한다.
- 카드를 hidden widget drop zone에 놓으면 숨기고, tray item을 grid에 놓으면 해당 위치로 복원한다. Enter 복원과 keyboard 이동·resize도 유지한다. 저장 중/성공/실패/충돌은 `aria-live` status로 알린다.
...
- 지원 viewport 기준은 768px과 1280px이며 768px 미만 화면은 지원하지 않는다.
- 1280px 이상 Overview는 저장 가능한 12열 grid이며 이동과 resize를 허용한다.
- 768~1279px Overview는 저장된 데스크톱 순서를 유지하는 자동 6열 grid다.
- `html`, `body`, `#root`의 최소 지원 폭은 768px이다.
...
- Overview layout: desktop widgets, revision, last committed layout, edit baseline, hidden widgets, save status/conflict
...
- Layout storage: navigation compact는 localStorage, Overview layout은 사용자별 Backend API
...
- Overview desktop: `react-grid-layout` 12-column grid, 12px gap, 54px row height
- KPI 기본 배치: 6개가 각 2열을 사용하는 strip
...
| Width | Layout |
| --- | --- |
| 1280px 이상 | 저장 가능한 12-column Overview, left icon rail |
| 768~1279px | 저장 순서 기반 읽기 전용 자동 6-column, rail은 상단 navigation으로 이동 가능 |

최소 지원 폭은 768px이다. 768px 미만 viewport를 위한 별도 navigation, 1-column layout 또는 detail flow는 제공하지 않는다.

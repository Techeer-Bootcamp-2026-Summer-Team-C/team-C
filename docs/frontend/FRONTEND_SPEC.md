...
- `/` Overview는 active custom dashboard와 무관하게 고정 CSS grid와 Default 10개 block을 항상 전부 표시하며 편집할 수 없다. Fleet Distribution은 고정 관제 context이며 Custom Dashboard widget으로 추가하지 않는다. `/dashboards`의 명시적 Draft 편집기에서는 기존 9개 widget의 desktop drag, resize, 삭제를 제공하고 Save에서 사용자 layout 전체를 저장한다. 기존 Backend layout API와 PostgreSQL 데이터는 호환성을 위해 유지하되 Frontend에서 호출하지 않는다.
...
- Default dashboard panel은 drag, drop, resize, hide, restore와 edit mode를 제공하지 않는다. Overview는 custom dashboard를 렌더링하거나 active layout identity를 표시하지 않는다.
...
- `/dashboards` workbench는 정적 preview와 Draft builder를 본문에 표시한다. 저장 layout 선택·새 Dashboard·편집·삭제 control은 본문 box가 아니라 page header의 `Dashboard 설정` modal이 소유한다. 새로 만들기 또는 편집을 선택하면 modal을 닫고 저장 layout을 Draft로 복제하며 desktop edit mode에서만 drag/drop, resize와 삭제를 제공한다.

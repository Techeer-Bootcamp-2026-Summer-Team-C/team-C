...
- Draft의 pointer/keyboard geometry는 Widget min/max, 12열 × 256행, 비중첩 조건으로 보정한다. Click 추가는 상단 행의 왼쪽부터 빈칸을 채우고, pointer drag·resize 충돌은 주변 Widget을 밀지 않으며 palette drop은 기존 geometry를 보존한 채 신규 Widget만 가장 가까운 빈칸에 배치한다. 이동·resize stop은 Draft state만 바꾸며 Save에서 전체 이름/layout을 한 번 저장하고 Cancel은 저장 상태를 바꾸지 않는다.

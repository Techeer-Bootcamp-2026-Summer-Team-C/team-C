...
| `/login` | Login | 불필요 |
| `/` | Overview | 필요 |
| `/dashboards` | 사용자별 Dashboard layout 관리 | 필요 |
| `/alerts` | Alert 목록 | 필요 |
...
`/dashboards`는 Primary navigation 항목이 아니다. Overview toolbar의 톱니바퀴 `Dashboard 설정` link로 진입하며 route breadcrumb와 인증 경계는 유지한다.

제외 경계:

- 저장·공유 Report Center API는 만들지 않고 브라우저 Print/Save PDF modal만 제공한다.
- Failure Sink는 읽기 전용이며 웹 replay는 제공하지 않는다. 일반 DLQ가 존재하는 것처럼 표현하지 않는다.
...
- `localStorage`에는 navigation compact 상태, `edr.theme`, 인증 사용자 `userId`별 custom Overview dashboard와 active dashboard만 저장한다. 별도 locale key, token 또는 API response 저장을 만들지 않는다.
- `/` Overview는 active Dashboard를 읽기 전용으로 표시한다. Default가 active이면 고정 CSS grid와 10개 block을 전부 표시하고, Custom Dashboard가 active이면 저장된 9개 widget 조합을 적용하며 Default 복원 action을 제공한다. Fleet Distribution은 Default 관제 context이며 Custom Dashboard widget으로 추가하지 않는다. `/dashboards`의 명시적 Draft 편집기에서만 기존 9개 widget의 desktop drag, resize, 삭제를 제공하고 Save에서 사용자 layout 전체를 저장한다. 기존 Backend layout API와 PostgreSQL 데이터는 호환성을 위해 유지하되 Frontend에서 호출하지 않는다.
...
- Login 화면은 locale과 무관하게 현재 영어 UI와 `OWLBY / SINGLE TENANT`를 사용하며 selector를 표시하지 않는다. KO 사용자가 logout한 뒤에도 Login은 영어이고, 재로그인 시 응답의 Backend locale을 인증 영역부터 적용한다.
...
- AppShell 안에서 Overview, Alerts, Incidents, Events, Intelligence 사이를 이동할 때 마지막으로 유효했던 `timePreset`을 유지한다. `CUSTOM`은 유효한 `from`·`to`를 함께 유지한다. Endpoint, Operations와 Archive에는 이 조회 기간을 API filter로 전달하지 않으며 Archive의 restore UTC 범위와 섞지 않는다.
...
- KPI: Total Alert, Critical Alert, HIGH-level Endpoint, OPEN Incident. `HIGH-level`은 `highRiskEndpointCount`의 정확한 `HIGH` 등급 의미이며 `CRITICAL`을 포함하지 않는다.
...
- Fleet Distribution: `EndpointSummaryDto.risk.byLevel`의 Risk 분포와 `sensorHealth`의 현재 보고 상태. 현재 snapshot이며 time trend로 표현하지 않는다.
- Highest-risk Endpoint와 OPEN Incident queue
- EDR state를 포함한 10개 block을 고정 DOM 순서로 표시한다. Fleet Distribution은 서버 Endpoint Summary 집계만 사용하고 원본 Endpoint를 client에서 재집계하지 않는다.
...
- 1280px 이상: full-width `EDR State` strip, `KPI 4개`, 왼쪽 `Detection Activity`와 오른쪽 `Alert Severity + Fleet Distribution` stack, `Endpoint 1fr + Incident 1fr` 순서로 배치한다. 1720px 이상에서는 세 분석 panel을 `2.1fr + .82fr + .92fr` 한 행으로 펼친다.
- 1024–1279px: KPI와 분석 panel을 1–2열로 재배치한다.
- 1023px 이하: DOM reading order를 유지하며 1–2열로 줄인다.
- Default dashboard panel은 drag, drop, resize, hide, restore와 edit mode를 제공하지 않는다. Overview는 active Custom Dashboard를 읽기 전용으로 렌더링하고 active layout identity와 Default 복원 action을 표시한다.
- Overview toolbar의 `Dashboard 설정` link는 현재 Endpoint/time URL query를 보존해 `/dashboards`로 이동한다. Overview 자체에는 Dashboard selector, 생성·편집·삭제 modal을 만들지 않는다.
- `/dashboards` workbench는 정적 preview와 Draft builder를 본문에 표시한다. 저장 layout 선택은 곧 Overview 적용 대상을 바꾸며 새 Dashboard·편집·삭제 control은 본문 box가 아니라 page header의 `Dashboard 설정` modal이 소유한다. 새로 만들기 또는 편집을 선택하면 modal을 닫고 저장 layout을 Draft로 복제하며 desktop edit mode에서만 drag/drop, resize와 삭제를 제공한다. Builder header에는 Dashboard 이름 입력을 Save 바로 옆에 두고 이름과 Widget 1개 이상이 필요하다는 조건을 항상 표시한다.
- Draft의 pointer/keyboard geometry는 Widget min/max, 12열 × 256행, 비중첩 조건으로 보정한다. Click 추가는 상단 행의 왼쪽부터 빈칸을 채우고, pointer drag·resize 충돌은 주변 Widget을 밀지 않으며 palette drop은 기존 geometry를 보존한 채 신규 Widget만 가장 가까운 빈칸에 배치한다. 빈 Draft도 Grid drop surface를 렌더링하고 empty-state 안내는 drop event를 가로채지 않는다. 이동·resize stop은 Draft state만 바꾸며 Save에서 전체 이름/layout을 한 번 저장하고 Cancel은 저장 상태를 바꾸지 않는다.
- Palette item은 Widget 의미를 나타내는 semantic preview icon, 이름과 기본 geometry를 함께 표시한다. Draft에는 type별 Widget을 하나만 둘 수 있고 이미 배치한 type은 palette에서 숨기며 제거하면 다시 표시한다. 추가 후에는 새 Widget handle, 제거 후에는 복귀한 palette item으로 keyboard focus를 옮긴다. Context create/update 저장 경계도 중복 type을 차단하고 Storage normalization은 중복 type 중 첫 번째 유효 instance만 유지한다.
- Custom Widget body는 정상 content에 내부 scrollbar를 만들지 않는다. 외부 drag header와 중복되는 내부 title은 screen-reader label로 유지하되 시각적으로 반복하지 않고, KPI는 값·설명을 compact하게 배치한다. EDR state, Detection Activity, Alert Severity와 최대 5개 queue는 registry의 콘텐츠 안전 최소 geometry 아래로 resize할 수 없다.
- Frontend는 Backend dashboard layout GET/PUT/DELETE를 호출하지 않고 Backend 저장 layout 응답을 화면과 병합하지 않는다. Custom dashboard는 사용자별 browser localStorage만 사용한다.
...
- 마지막 unresolved Alert를 `RESOLVED`로 저장해 현재 filter queue가 0건이 되면 마지막 Alert 상세를 남기지 않고 queue 완료 상태와 목록 복귀 action을 표시
...
- 읽기 전용 Failure Sink table과 status/stage/retryable filter
...
이 제품은 빠르게 이상 상태를 파악하고 근거 데이터로 이동할 수 있는 조용한 보안 관제 콘솔이다. 시그니처는 244px 기본 Sidebar, Signal → Evidence → Decision 상단 경로, royal-blue active navigation, near-black dark baseline과 semantic light surface, 제한된 상태 색상이다. 화면은 마케팅 Dashboard처럼 과장하지 않고, 숫자·상태·최근성·다음 이동 경로를 우선한다.
...
## 17. Color — 값 확정 / Case 2 final-selected

Case 2 dark를 기본 semantic token으로 유지하고 같은 역할 이름의 light token을 제공한다. 인증된 AppShell에서 theme를 전환하며 `edr.theme`에 `dark` 또는 `light`만 저장한다. 저장값이 없거나 손상되면 dark로 복구한다. Theme는 Login을 포함한 모든 route에 적용하지만 Login에는 별도 toggle을 제공하지 않는다.

현재 구현 기준은 `frontend/src/styles/tokens.css`의 dark semantic token이다. component에는 raw color를 추가하지 않고 semantic token만 참조한다.

| 역할 | Token | 값 | 사용 |
| --- | --- | --- | --- |
| Page background | `--surface-canvas` | `#09090B` | 전체 배경 |
| Shell background | `--surface-shell` | `#17161B` | rail, top chrome |
| Surface primary | `--surface-panel` | `#0C0C0F` | 기본 panel |
| Surface raised | `--surface-raised` | `#1B1B20` | popover, tooltip |
| Surface inset | `--surface-inset` | `#07070A` | chart, inspector inset |
| Surface hover | `--surface-hover` | `#202126` | interactive hover |
| Surface selected | `--surface-selected` | `#1D3989` | selected row/context |
| Border default | `--border-default` | `#5D5E67` | control, raised surface border |
| Border panel | `--border-panel` | `#34353C` | card, panel outer border |
| Border subtle | `--border-subtle` | `#25262B` | divider, table row |
| Text primary | `--text-primary` | `#F5F5F6` | heading, primary value |
| Text secondary | `--text-secondary` | `#C5C6CC` | description, metadata |
| Text tertiary | `--text-tertiary` | `#9698A1` | quiet label |
| Focus | `--focus-ring-color` | `#4C85FF` | keyboard focus ring |
| Accent | `--accent-primary` | `#2563E9` | primary action, active navigation |
| Critical | `--status-critical` | `#F04444` | CRITICAL, RED, unavailable |
| High | `--status-high` | `#F97316` | HIGH severity/risk |
| Medium | `--status-medium` | `#EAB308` | MEDIUM severity/risk |
| Low | `--status-low` | `#60A5FA` | LOW severity/risk |
| Warning | `--status-warning` | `#E7D56A` | YELLOW, offline, degraded |
| Success | `--status-success` | `#22C55E` | GREEN, online, healthy |
| Info | `--status-info` | `#38BDF8` | OPEN, IN_PROGRESS |
| Neutral | `--status-neutral` | `#A1A1AA` | CLOSED, RETIRED, UNKNOWN |
...
- Royal blue accent는 action과 active navigation에, 밝은 blue는 focus ring에 사용한다. Violet은 Detection Activity의 Alerts series 외에는 사용하지 않는다.
...
- Detection Activity는 Events `#2563E9`, Alerts `#7C83FD`, Open Incidents `#16A249`로 상태색과 분리한다.
- 일반 UI gradient는 금지하고 Detection Activity의 낮은 opacity area fill만 허용한다.
- 새 raw hex는 이 문서에 token을 먼저 추가한 뒤 사용한다.

## 18. Typography — 값 확정 / self-hosted

본문·제목·수치·metadata·code의 위계와 값을 확정한다.

### Font stack

- English/UI/data: `Inter Variable`
- Korean: `Pretendard Variable`
- Technical/log: `IBM Plex Mono` 400/500/600
- 모든 font는 Frontend bundle의 WOFF2를 사용하고 `font-display: swap`으로 제공한다. 외부 CDN 요청은 금지한다.

### Type scale

| Token | Size | Weight | Line height | 사용 |
| --- | ---: | ---: | ---: | --- |
| `--type-service-name` | 12px | 600 | 1.25 | Service identity |
| `--type-edr-score` | 32px | 600 | 1 | EDR 종합 점수 |
| `--type-kpi-primary` | 30px | 600 | 1.15 | 주요 KPI |
| `--type-kpi-secondary` | 20px | 600 | 1.1 | 보조 KPI, chart center |
| `--type-section-title` | 20px | 600 | 1.25 | Section title |
| `--type-panel-title` | 14px | 600 | 1.3 | Panel title |
| `--type-body` | 13px | 400 | 1.55 | 기본 본문 |
| `--type-label` | 12px | 500 | 1.35 | control label |
| `--type-table-body` | 12px | 400 | 1.45 | table body |
| `--type-table-header` | 11px | 600 | 1.35 | table header |
| `--type-meta` | 11px | 500 | 1.45 | metadata |
| `--type-code` | 12px | 400 | 1.45 | ID, hash, path, IP |

규칙:

- 11px는 table header, badge와 짧은 metadata에만 사용한다.
- weight는 400~700 범위로 제한한다.
...
- **Identity**: Overview root top bar와 Sidebar는 확정 서비스명 `OWLBY`와 box 없는 `Evidence O` compact symbol을 표시한다. 다른 route는 `OWLBY`를 root로 하는 breadcrumb와 page title을 유지한다.
...
| Skeleton | 1250ms | ease-in-out | 정적인 surface의 opacity pulse |
...
- 기본 panel과 card에는 shadow를 사용하지 않는다. Tooltip, popover와 modal처럼 부유하는 surface에만 제한한다.

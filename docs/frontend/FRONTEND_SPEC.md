...
- `/` Overview는 active custom dashboard와 무관하게 고정 CSS grid와 기존 Default 9개 block을 항상 전부 표시하며 편집할 수 없다. `/dashboards`의 명시적 Draft 편집기에서만 현재 9개 widget의 desktop drag, resize, 삭제를 제공하고 Save에서 사용자 layout 전체를 저장한다. 기존 Backend layout API와 PostgreSQL 데이터는 호환성을 위해 유지하되 Frontend에서 호출하지 않는다.

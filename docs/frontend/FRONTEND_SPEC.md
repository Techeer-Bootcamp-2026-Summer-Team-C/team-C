...
- JWT access token, `UserDto`, 실제 만료시각은 현재 탭의 `sessionStorage`에 저장해 새로고침과 개발 서버 reload 후 복구한다.
- 그 외 API response와 domain data는 `localStorage`나 `sessionStorage`에 저장하지 않는다.
- `localStorage`에는 마지막 primary route와 navigation compact 상태만 저장한다. Overview layout은 JWT 사용자별 Backend API와 PostgreSQL `user_dashboard_layouts`에 저장한다.
...
- active Alert 기반 읽기 전용 Response Guidance 요약
- `lastRefreshedAt`

KPI와 chart 선택은 관련 목록 route로 이동하며 가능한 경우 URL filter를 설정한다.

Overview의 모든 콘텐츠 카드 23개는 안정 ID registry로 관리한다. 헤더, sidebar, 사용자 menu, route와 위젯 내부 제목·버튼·범례는 layout 단위가 아니다. 기본 화면을 먼저 표시하고 저장 layout 응답을 registry와 병합한다.
...
- 제품 REST API 수는 Dashboard 23개 + Collector 3개 = 26개다.

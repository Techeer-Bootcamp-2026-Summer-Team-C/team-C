...
### TCD-14. Frontend 운영 흐름 후속 개선

- 상태: 완료
- 사용자 결정: 7번 날짜 입력 locale과 9번 Incident Evidence 연동 선택은 보류한다. 공통 기간 filter의 route 간 유지, Alert Queue 종료 상태, Endpoint Egress Topology 관계선 집약과 control 대비, Dashboard 이름 입력 안내, Custom Dashboard의 Overview 적용, Archive filter 정렬을 수정한다.
- Dashboard 결정: `/`는 현재 사용자가 선택한 Custom Dashboard를 읽기 전용으로 표시하고 Default를 명시적으로 다시 적용할 수 있게 한다. Default 자체의 immutable 성격, `/dashboards` Draft Save/Cancel, 사용자별 browser-local 저장과 Backend layout API 비호출은 유지한다. 이 결정은 TCD-10의 고정 Overview 적용 제약을 대체한다.
- 계약 경계: Backend, API_SPEC, OpenAPI, generated schema, DTO와 DB를 변경하지 않는다. 시간 filter는 각 route URL을 source of truth로 유지하고 Archive의 UTC restore 범위에는 일반 조회 preset을 전달하지 않는다.
- 구현 결과: AppShell이 마지막 유효 기간 범위를 기억해 Overview·Alerts·Incidents·Events·Intelligence 이동 URL에 전달한다. 마지막 unresolved Alert을 해결하면 Queue 전체를 완료 상태로 교체한다. Topology는 같은 Endpoint-target의 protocol별 edge를 한 관계선으로 집약하되 원본 semantic table은 protocol별 evidence를 유지한다. graph control은 theme과 무관하게 흰 배경·검은 glyph로 고정했다. Dashboard builder는 이름 입력과 필수 안내를 Save 옆에 배치했고, 저장한 Custom Dashboard를 Overview에 읽기 전용으로 적용하며 `Use Default`로 복원한다. Archive filter 입력과 action의 수직 기준선을 맞췄다.
- 변경 파일: `frontend/src/components/AppShell.tsx`, `frontend/src/lib/url.ts`, `frontend/src/pages/AlertDetailPage.tsx`, `frontend/src/features/intelligenceOperations.ts`, `frontend/src/components/TopologyGraph.tsx`, `frontend/src/pages/IntelligencePage.tsx`, `frontend/src/features/overviewLayout/OverviewDashboardWorkspace.tsx`, `frontend/src/pages/ArchivesPage.tsx`, 관련 translation·CSS·5개 regression test, `docs/frontend/DESIGN.md`, `docs/frontend/FRONTEND_SPEC.md`, 이 작업지시서.
- 자동 검증: targeted `npm test -- app-shell-foundation.test.tsx policy-url.test.ts alert-workbench.test.tsx intelligence-operations-archives.test.tsx custom-dashboard.test.tsx` → 5 files / 44 tests passed. `npm run openapi:check`, `npm run typecheck`, `npm run lint`, 전체 `npm test` → 25 files / 143 tests, `npm run build`, `git diff --check`가 모두 통과했다.
- Desktop browser QA: 수정 source를 기존 8080 Backend proxy와 연결해 1280 × 900과 1440 × 900 dark theme에서 확인했다. 7일 범위가 Overview→Alerts→Incidents와 비기간 route 경유 후 Events에 유지되고 Archive에는 전달되지 않았다. 새 Dashboard 화면에서 이름 필드·필수 안내·Save가 같은 action row에 배치되며, 검사용 1-Widget Dashboard를 저장해 Overview read-only 적용, `Use Default` 복원, 검사용 Dashboard 삭제까지 확인했다. Topology는 `FINANCE-MAC-02 → storage.example.com`의 HTTP/TLS/HTTPS를 `HTTP +2 · E6 A6` 한 선으로 표시하면서 table 3행을 유지했고, dark theme control은 흰 bar·검은 icon으로 식별됐다. Archive의 Endpoint IDs·From·To 입력과 Clear action은 두 viewport에서 같은 기준선으로 정렬됐다. 가로 넘침이나 깨진 재배치는 보이지 않았고 QA browser와 전용 Vite server를 종료했다.
- 범위/남은 검증: 7번 native date input locale과 9번 Incident Evidence 선택은 사용자 결정대로 수정·검증하지 않았다. Browser wrapper에서 console log capability가 제공되지 않아 console warning/error 직접 수집은 미검증이다. 모바일 시각 QA와 모바일 전용 builder UX는 사용자 요청 범위에서 제외했다.

# EDR_C 발표용 목데이터 실행 Runbook

이 Runbook의 데이터와 수치는 발표용으로 재구성한 local/QA 목데이터다. production 실측이나 실제 침해 기록으로 표현하지 않는다.
...
PostgreSQL과 ClickHouse의 local/QA 데이터를 초기화하고 `3 Endpoint / 64 Event / 3 Alert / 2 Incident`를 생성한다.
...
로그인은 `frontend-admin / frontend-admin-password`를 사용한다. manifest에는 실행 후 결정된 Endpoint·Alert·Incident ID와 발표 URL이 기록된다.

## 2. 발표 클릭 순서

1. manifest의 `urls.overview`를 열고 `LATEST_24H`에서 64 Event, 3 Alert, 2 open Incident를 확인한다.
2. Highest-risk Endpoint `DEMO-STUDENT-WIN-07`을 연다.
3. Endpoint 화면의 `최근 Event 열기`를 눌러 24개 전체 Timeline을 확인한다.
4. Overview 또는 Incident 목록에서 manifest의 `powershellIncidentId`에 해당하는 Incident를 연다.
5. `Attack Timeline`과 연결 Alert 표에서 같은 `PROC_POWERSHELL_ENCODED` Alert 2개를 확인한다.
6. 두 Alert의 원본 Event를 열어 같은 Endpoint와 같은 30분 window인지 확인한다.
7. `suspicious-egress` Incident와 `NET_SUSPICIOUS_EGRESS` Alert가 별도로 존재하는지 확인한다.

## 3. DNS correctness profile

이 profile도 destructive reset이므로 presentation profile과 동시에 유지되지 않는다. 발표 직전이 아니라 Screenshot·녹화 준비 시 별도로 실행한다.
...
기본 `direct-seed`는 발표 상태를 빠르고 결정적으로 복원한다. Event는 ClickHouse repository로, Alert·Incident는 현재 Rule loader와 Detection engine 및 PostgreSQL repository로 생성한다. Kafka 처리 증거로 사용하지 않는다.
...
- 최종본: `output/playwright/presentation-demo-final.webm`
- 규격: WebM, 1280 x 720, 76.64초
- 포함 동선: Overview 64 Event/3 Alert/2 Incident → 메인 Endpoint → 24 Event Timeline → PowerShell Incident → 동일 Rule Alert 2개와 Attack timeline
- 로그인 비밀번호 입력 장면, `.env`, 인증서 private key, production 화면은 포함하지 않았다.

단, 고정 anchor가 현재 시각 기준 24시간 밖이면 Overview의 `LATEST_24H`에는 보이지 않는다. 고정 녹화에서는 manifest의 CUSTOM 범위 URL을 사용하고, 라이브 발표 직전에는 `--anchor now`로 재생성한다.

...
- `NET_SUSPICIOUS_EGRESS`: Suspicious TLS destination detected
...
- `NET_SUSPICIOUS_EGRESS`
  - `event_type: L7_EVENT`
  - `l7_protocol: TLS`, configured `tls_sni` pattern
  - `correlation_key: powershell-tls-egress-chain`
  - `window_seconds: 1800`

두 Rule은 의도적으로 같은 correlation key와 window를 사용한다. 따라서 같은 Endpoint와 같은 30분 고정 bucket의 PowerShell Alert와 TLS SNI Egress Alert는 하나의 Incident를 공유한다. 이는 시간 기반 correlation이며 Process가 해당 TLS 통신을 만들었다는 인과관계를 증명하지 않는다.

### 6.2 코드 기준 권장 시연

한 Incident에 여러 Rule의 Alert가 연결되는 모습은 다음 시나리오로 보여준다.

```text
동일 Endpoint
→ 같은 30분 고정 bucket에서 Encoded PowerShell Event가 두 번 발생
→ 같은 bucket에서 configured SNI를 가진 TLS ClientHello Event가 한 번 발생
→ PROC_POWERSHELL_ENCODED Alert 두 개와 NET_SUSPICIOUS_EGRESS Alert 한 개 생성
→ 동일 powershell-tls-egress-chain Incident에 연결
```

같은 Incident에서 세 Alert를 함께 보여주되, 시간 근접성 이상의 인과관계를 검증했다고 말하지 않는다.

### 6.3 현재 QA 시드 주의사항

발표용 `tools/seed_presentation_demo.py`는 현재 Rule v2를 평가해 PowerShell Alert 2개와 TLS SNI Egress Alert 1개를 동일 `powershell-tls-egress-chain` Incident에 연결한다. 발표 전에 manifest의 3 Alert·1 Incident와 Dashboard 결과가 일치하는지 검증한다.

과거 `tests/seed_frontend_qa.py` 화면이나 기존 영상은 현재 presentation profile 결과를 재현한 증거가 아니다. Endpoint, Rule version, correlation key와 count를 다시 확인하지 않은 자료를 실제 Worker 결과처럼 사용하지 않는다.
...
- 같은 Endpoint에서 발생한 Encoded PowerShell Alert 2개와 TLS SNI Egress Alert 1개
- 같은 `powershell-tls-egress-chain` correlation key와 30분 고정 bucket
- 동일 Incident에 연결된 세 Alert

권장 멘트:

> 같은 Endpoint의 Encoded PowerShell 탐지 두 건과 TLS SNI 기반 외부 통신 탐지 한 건이 명시적으로 같은 correlation key와 30분 구간을 사용해 하나의 Incident에 연결됐습니다. 이는 시간 기반 조사 단서이며 인과관계 증명은 아닙니다.

최종 리허설에서는 실제 Detection Worker의 결과, 시연 fixture와 발표 멘트가 정확히 일치하는지 확인한다.
...
### 9.3 Kafka partition 기준

- 현재 `backend/kafka.py` 기본값: topic당 2 partitions
- 현재 `compose.yaml` 기본값: topic당 2 partitions
- `docs/architecture/TECH_STACK.md`: topic당 기본 2 partitions

발표에서 수치가 필요하면 로컬 기본값 2라고 말한다. Worker는 설정된 partition 수 범위에서 확장할 수 있으며 자동 scaling이나 무제한 확장을 구현했다고 말하지 않는다.
...
- 시간 기반 PowerShell·TLS SNI correlation을 Process와 통신의 인과관계 증명이라고 설명한다.
...
### PowerShell과 TLS SNI egress를 하나의 Incident로 연결하는가?

그렇다. 두 Rule이 `powershell-tls-egress-chain`과 1,800초 window를 명시적으로 공유하므로 같은 Endpoint·고정 bucket에서 하나의 Incident에 연결된다. 현재 구현은 시간 근접성을 이용한 좁은 cross-rule correlation이며 Process와 TLS 통신의 인과관계를 증명하지 않는다.
...
- [ ] Kafka partition 수를 언급한다면 코드·Compose·TECH_STACK의 기본값 2와 일치

### 데모

- [ ] 발표 전 목데이터 생성 완료
- [ ] PowerShell Alert 2개와 TLS SNI Egress Alert 1개가 동일 `powershell-tls-egress-chain` Incident에 연결되는지 확인
- [ ] Incident 안의 세 Alert가 같은 Endpoint·correlation key·fixed window에서 실제 Worker 동작으로 생성됐는지 확인
- [ ] 시간 기반 correlation을 Process와 통신의 인과관계로 설명하지 않는지 확인
...
- 시연용 탐지 규칙: `rules/process/proc_powershell_encoded.v2.yaml`, `rules/network/net_suspicious_egress.v2.yaml`
...
7. 작성된 8분 발표 대본을 실제 배포 상태에 맞춰 최종 검토 — `docs/presentation/PRESENTATION_SCRIPT.md`
8. 백업 영상과 PDF 제작
9. 최소 3회 전체 리허설 및 시간 조정
10. Kafka partition 기본값 2가 발표 자료에 일관되게 반영됐는지 확인
11. presentation profile의 Rule v2·3 Alert·1 Incident 결과를 리허설에서 재검증

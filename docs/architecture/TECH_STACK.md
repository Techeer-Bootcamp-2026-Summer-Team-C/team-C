...
| Dashboard API | Python, FastAPI, Uvicorn, PyArrow | login 포함 23개 Dashboard Backend REST API, Endpoint Risk/전역 EDR 상태 계산, 사용자별 layout, restored Parquet 직접 조회, Swagger |
...
Local Compose는 Event Storage Worker와 Detection Worker를 각각 1개만 실행하며 각 consumer가 3개 partition을 모두
...
- 제품 REST API는 Dashboard Backend 25개 + Collector 3개 = 28개다.
...
- Dashboard는 React 19, TypeScript, Vite로 구현하고 REST polling을 사용한다.

...
| 실행 | Swift CLI + system LaunchDaemon |
...
tcpdump는 `-w -`로 stdout에 packet stream을 쓰고 Agent가 즉시 parsing한다. 디스크 spool이나 PCAP 파일을 만들지 않는다. 현재 prototype은 collection cycle마다 tcpdump를 시작하므로 system LaunchDaemon 전체가 root로 실행된다. config/private key/state directory는 root ownership과 제한된 mode를 startup에서 검사하고 plist는 `umask 077`을 적용한다. 별도 privileged packet-capture helper와 unprivileged main process로의 권한 분리는 아직 구현하지 않았으며 privilege drop이 있다고 설명하지 않는다.
...
`telemetry.raw`, `telemetry.validated`는 각각 기본 2개 partition과 local replication factor 1을 사용한다. 기존
Topic이 1개 partition이면 startup에서 2개로 증가시키며 더 많은 partition은 축소하지 않는다. Collector와
Event Storage Worker는 `endpointId`를 Kafka key로 사용해 같은 Endpoint의 이벤트를 같은 partition에 유지한다.
Local Compose는 Event Storage Worker와 Detection Worker를 각각 1개만 실행하며 각 consumer가 2개 partition을 모두
담당한다. Worker 수는 부하에 따라 partition 수 이하에서 확장한다.

### 5.1 로컬 Compose와 실제 배포 경계

로컬 개발환경은 Docker Desktop의 단일 `edr-c-local` Compose 프로젝트다. `docker compose up -d --build --wait` 한 번으로 다음 11개 서비스를 실행한다.

| 로컬 Compose 서비스 | 역할 |
| --- | --- |
| `postgres`, `clickhouse`, `kafka`, `minio` | 로컬 데이터 인프라 |
...
실제 배포는 Compose 컨테이너 수와 무관하게 다음 7개 서버 경계로 나눈다.
...
로컬 `frontend`와 MinIO는 각각 Vercel과 Amazon S3의 개발용 대응물이다. `cert-init`, `app-init`은 로컬 bootstrap job이며 별도 운영 서버 경계가 아니다.

### 5.2 인증·사용자 Bootstrap CLI
...
- 등록 API를 제외한 heartbeat·telemetry는 전달된 fingerprint가 해당 Endpoint의 `is_delete=false`, `revoked_at IS NULL`, `issued_at <= now()`, `expires_at > now()`인 활성 `agent_auth_keys` row와 일치하고 Endpoint가 `RETIRED`가 아니어야 한다. Nginx는 외부 신원 헤더를 제거하고 TLS 검증을 통과한 escaped PEM certificate만 전달한다. Backend는 이 certificate의 단일 URI SAN, subject, SHA-256 fingerprint, notBefore, notAfter를 직접 읽고 인증서 시각을 UTC로 변환한다.
...
conditions:
  all:
    ...
    - field: command_line
      operator: regex
      value: '(?i)(?:^|\s)[-/](?:encodedcommand|enc)(?:\s|$)'
severity: HIGH
risk_score: 85
mitre:
  tactic_code: TA0002
...
MVP operator는 `eq`, `neq`, `contains`, `regex`, `in`, `cidr_contains`, `gt`, `gte`, `lt`, `lte`다. RuleV1 `rule_name`, `alert_title`, `alert_summary`는 required non-empty string이며 Detection Worker가 템플릿 처리 없이 Alert의 `rule_name`, `title`, `summary`에 그대로 snapshot한다. RuleV1 `risk_score`는 0~100 number다. 활성 Rule의 condition field는 해당 `event_type`의 정규화된 payload field여야 하며, 다른 event type의 field를 참조하면 Rule readiness를 실패시킨다. 모든 `enabled=true` RuleV1은 `mitre.tactic_code`, `mitre.technique_code`를 가져야 하며 두 code가 `mappings/mitre_attack.yaml`에 없거나 누락되면 Worker readiness를 실패시킨다. `enabled=false` Rule은 `mitre`를 생략하고 `incident.enabled=false`이면 `correlation_key`, `window_seconds`를 생략한다. YAML에는 MITRE code만 저장하고 Backend가 고정 mapping 파일에서 tactic/technique name을 변환해 Alert의 MITRE code/name 4개 NOT NULL 컬럼을 채운다. 새 MITRE 테이블은 만들지 않는다.
...
Alert는 `(event_id, rule_code, rule_version)`으로 멱등 생성한다. Incident는 `(endpoint_id, correlation_key, window_start_at)`으로 UPSERT하며 생성 시 `OPEN`이다. 서로 다른 활성 Rule이 의도적으로 같은 `correlation_key`와 `window_seconds`를 선언한 경우에만 같은 Endpoint·고정 시간 window에서 하나의 Incident를 공유한다. 같은 key에 서로 다른 window 크기를 선언하면 readiness를 실패시키며, Endpoint가 다르거나 key/window가 다르면 합치지 않는다. 현재 `PROC_POWERSHELL_ENCODED`와 TLS SNI 기반 `NET_SUSPICIOUS_EGRESS`만 `powershell-tls-egress-chain`을 공유한다. 이는 좁은 시간 기반 상관관계이며 프로세스와 연결의 인과관계를 증명하지 않는다. 같은 Detection Worker process의 60초 periodic task가 `status=OPEN AND status_overridden=false AND window_end_at <= now()`를 확인해 `CLOSED`로 바꾸고 `closed_at=window_end_at`을 기록한다. `ADMIN`, `ANALYST`는 Incident 상태 변경 REST API로 `OPEN`과 `CLOSED`를 전환할 수 있고, 수동 변경된 상태는 `status_overridden=true`로 기록되어 다음 수동 변경까지 자동 종료 작업이 덮어쓰지 않는다. 같은 process의 별도 30초 task는 `RETIRED`가 아닌 Endpoint의 `last_seen_at`을 검사해 2분 미수신이면 `OFFLINE`으로 바꾼다. Heartbeat는 `RETIRED`가 아닌 Endpoint를 `ONLINE`으로 만들며 `RETIRED`는 두 상태보다 우선하고 자동 변경하지 않는다.
...
| PostgreSQL | users, user dashboard layouts, endpoints, auth keys, audit, alerts, incidents, timestamp bucket catalog |
...
- Dashboard는 React 19, TypeScript, Vite로 구현하고 route 진입·filter 변경·사용자 action 기반 REST query를 사용한다. timer, focus, reconnect 기반 자동 polling은 사용하지 않는다.
...
-> Mac에서 pull
-> swift build/test
-> tcpdump stdout/FSEvents/system LaunchDaemon 실제 검증

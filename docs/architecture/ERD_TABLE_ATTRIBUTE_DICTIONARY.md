...
이 문서는 ERDCloud 업로드 파일 `ERD_FINAL.sql`과 `ERD_FINAL_NO_COMMENTS.sql`의 11개 테이블·193개 속성을 설명하는 데이터 사전이다. ERDCloud DDL은 importer 호환을 위해 MySQL 계열 타입으로 표현하지만, 아래 속성 표의 타입은 실제 실행 저장소인 SQLite, PostgreSQL, ClickHouse 타입을 기준으로 한다.
